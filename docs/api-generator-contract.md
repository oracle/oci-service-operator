...
- If the published OSOK kind must keep an existing checked-in name, record the
  ...
  code.
- Existing compatibility-locked kind names should remain recorded in
  `services.yaml`, not hardcoded in generator logic.
...
- Resource overrides may pin individual kinds to `manual` so existing webhook
  seams stay explicit during migration.
...
`keymanagement/ReplicationStatus`, `keymanagement/WrappingKey`,
`streaming/Cursor`, `streaming/Group`, `streaming/GroupCursor`, and
`streaming/Message`. Resources that still keep explicit adapters or other
handwritten parity seams stay authoritative through those checked-in files
until later rollout work removes them.
