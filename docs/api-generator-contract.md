...
| `pkg/servicemanager/**` | Service-manager generator when `generation.serviceManager.strategy=generated` | Generated scaffolds are baseline implementations; existing manual layouts may be referenced through `generation.resources[].serviceManager.packagePath`. Generated baseline package shapes now include `*_runtimehooks_generated.go`, `*_serviceclient.go`, and `*_servicemanager.go`. |
...
- Generated baseline packages contain `<file-stem>_runtimehooks_generated.go`,
  `<file-stem>_serviceclient.go`, and `<file-stem>_servicemanager.go`.
