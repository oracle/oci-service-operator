...
| `generation.resources[].webhooks.strategy` | Optional per-kind webhook ownership override: `manual` or `none`. When omitted, the kind inherits the service-level webhook strategy. |
...
| `api/<group>/<version>/*_webhook.go` | Manual | Webhook code stays manual in this epic; the generator only records ownership seams through `generation.webhooks`. The current repo only keeps these files for `database/AutonomousDatabases`. |
| `api/<group>/<version>/webhook_suite_test.go` | Manual | Remains manual with webhook code. The current repo only keeps this suite harness for `api/database/v1beta1`. |
| `controllers/<group>/*_controller.go` | Controller generator when the effective per-kind controller strategy resolves to `generated` | Manual files stay authoritative until rollout switches from `manual` or `none`, and resource-level overrides may keep selected published kinds out of generated runtime even when the service-level default is `generated`. |
...
| `pkg/servicemanager/**` | Service-manager generator when the effective per-kind service-manager strategy resolves to `generated` | Generated scaffolds are baseline implementations; existing manual layouts may be referenced through `generation.resources[].serviceManager.packagePath`, and resource-level overrides may keep selected published kinds out of generated runtime. |
| `internal/registrations/*.go` | Manual runtime bridge | Handwritten bridge code preserves explicit adapters and webhook hooks for published resources that still keep checked-in seams beside the generated group files. Today the explicit webhook hook is limited to `AutonomousDatabases`. |
| `internal/registrations/<group>_generated.go` | Registration generator when `generation.registration.strategy=generated` | Registers the full API group scheme, then wires controller setup and service-manager factories only for published kinds whose effective controller and service-manager strategies resolve to `generated`. |
...
- Published resources that still keep checked-in adapters or webhook seams
  bridge into this contract through handwritten files under
  `internal/registrations/` without changing the shared manager bootstrap
  shape in `main.go`.
...
checked-in repo that is every configured service group. Published resources
that keep checked-in adapters, explicit webhook seams, or API-only runtime
overrides remain `controller-backed`; the profile describes install posture,
not whether every published kind in the group currently has generated
controller and service-manager outputs. The current API-only runtime exceptions
are `keymanagement/Key`, `keymanagement/KeyVersion`,
`keymanagement/ReplicationStatus`, `keymanagement/WrappingKey`,
`streaming/Cursor`, `streaming/Group`, `streaming/GroupCursor`, and
`streaming/Message`.
...
emits generated registration outputs for every configured service group, and
emits controller/service-manager outputs only for published kinds whose
effective per-kind strategies resolve to `generated`. Today the API-only
runtime exceptions are `keymanagement/Key`, `keymanagement/KeyVersion`,
`keymanagement/ReplicationStatus`, `keymanagement/WrappingKey`,
`streaming/Cursor`, `streaming/Group`, `streaming/GroupCursor`, and
`streaming/Message`. Resources that still keep explicit adapters or the
remaining `database/AutonomousDatabases` webhook path stay authoritative
through those checked-in seams until later rollout work removes them.
...
- full replacement of the remaining `database`, `mysql`, and `streaming`
  checked-in adapters and the explicit `database/AutonomousDatabases` webhook
  seam
