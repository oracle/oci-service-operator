...
- `generation.resources[].kind` uses the final published OSOK kind after any
  compatibility overrides are applied.
...
Manual carve-outs are still deliberate. Webhook files when present, suite-test
harnesses, and direct `main.go` edits remain outside generator ownership even
when controller and service-manager generation is enabled.
...
surfaces while keeping any remaining manual webhook seams explicit.
...
7. Move a service from `crd-only` to `controller-backed` in `services.yaml`
   ...
   resources with preserved checked-in seams, keep any remaining
