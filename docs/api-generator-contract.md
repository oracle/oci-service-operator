...
- `generation.resources[].kind` uses the published OSOK kind after any
  compatibility rename is applied.
- Resource-level rollout inherits the corresponding service-level strategy when
  the override omits `strategy`.
- `generation.resources[].sdkName` is only needed when the published OSOK kind
...
- Existing locked compatibility kinds should remain recorded in
...
| `api/<group>/<version>/*_webhook.go` | Manual when present | Webhook code is not generator-owned; `generation.webhooks` only records ownership seams. |
...
| `internal/registrations/*.go` | Manual runtime bridge | Handwritten bridge code can adapt any remaining manual runtime seams to the shared registration/runtime contract until generated registration rollout lands. |
...
- Remaining manual runtime seams can bridge into this contract through
  handwritten files under `internal/registrations/` without changing the
...
checked-in repo this describes install posture, not whether every published
kind in a group currently has fully generated controller and service-manager
outputs. Groups that still keep checked-in adapters or API-only runtime
overrides remain `controller-backed`.
