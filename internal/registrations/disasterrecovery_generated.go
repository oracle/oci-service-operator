			...
			if err := (&disasterrecoverycontrollers.DrProtectionGroupReconciler{
				Reconciler: NewBaseReconciler(
					...
					func(deps servicemanager.RuntimeDeps) servicemanager.OSOKServiceManager {
						return disasterrecoverydrprotectiongroupservicemanager.NewDrProtectionGroupServiceManagerWithDeps(deps)
					},
				),
			}).SetupWithManager(ctx.Manager); err != nil {
				return fmt.Errorf("setup DrProtectionGroup controller: %w", err)
			}
			return nil
		},
	})
