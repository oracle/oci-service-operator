...
func generatedDocsPaths(root string) ([]string, error) {
	...
	if err := filepath.WalkDir(docsRoot, func(path string, d os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		...
		generated, err := isGeneratedMarkdownFile(path)
		if err != nil {
			return err
		}
		if !generated {
		...
		return nil, fmt.Errorf("walk checked-in generated docs outputs: %w", err)
	}
...
func isGeneratedMarkdownFile(path string) (bool, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return false, fmt.Errorf("read generated markdown candidate %q: %w", path, err)
	}
	firstLine := string(content)
	if newline := strings.IndexByte(firstLine, '\n'); newline >= 0 {
		firstLine = firstLine[:newline]
	}
	return strings.HasPrefix(strings.TrimSpace(firstLine), "<!-- Code generated by cmd/sitegen"), nil
}
