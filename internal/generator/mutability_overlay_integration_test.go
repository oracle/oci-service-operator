...
func TestGenerateMutabilityOverlayKeepsUnresolvedASTFields(t *testing.T) {
	t.Parallel()
...
`)
  ...
  "mutation": {
    "mutable": ["display_name", "unsupported_repo_only"],
    "forceNew": ["compartment_id", "unsupported_force_new"],
    "conflictsWith": {
...
`)
	...
	if len(doc.Fields) != 4 {
		t.Fatalf("fields = %d, want 4", len(doc.Fields))
	}

	unresolvedMutable := findMutabilityOverlayField(t, doc.Fields, "unsupportedRepoOnly")
	if unresolvedMutable.CanonicalJoinKey != "unsupportedRepoOnly" {
		t.Fatalf("unsupportedRepoOnly canonicalJoinKey = %q, want %q", unresolvedMutable.CanonicalJoinKey, "unsupportedRepoOnly")
	}
	if unresolvedMutable.Merge.JoinStatus != mutabilityOverlayJoinUnresolved {
		t.Fatalf("unsupportedRepoOnly joinStatus = %q, want %q", unresolvedMutable.Merge.JoinStatus, mutabilityOverlayJoinUnresolved)
	}
	if unresolvedMutable.Merge.FinalPolicy != mutabilityOverlayPolicyUnknown {
		t.Fatalf("unsupportedRepoOnly final policy = %q, want %q", unresolvedMutable.Merge.FinalPolicy, mutabilityOverlayPolicyUnknown)
	}
	if unresolvedMutable.PathShape != mutabilityOverlayPathShapeScalar {
		t.Fatalf("unsupportedRepoOnly pathShape = %q, want %q", unresolvedMutable.PathShape, mutabilityOverlayPathShapeScalar)
	}
	if !containsDiagnostic(unresolvedMutable.Provenance.Notes, `did not resolve to exactly one canonical join key`) {
		t.Fatalf("unsupportedRepoOnly notes = %v, want unresolved-join detail", unresolvedMutable.Provenance.Notes)
	}

	unresolvedForceNew := findMutabilityOverlayField(t, doc.Fields, "unsupportedForceNew")
	if unresolvedForceNew.Merge.JoinStatus != mutabilityOverlayJoinUnresolved {
		t.Fatalf("unsupportedForceNew joinStatus = %q, want %q", unresolvedForceNew.Merge.JoinStatus, mutabilityOverlayJoinUnresolved)
	}
	if unresolvedForceNew.Merge.FinalPolicy != mutabilityOverlayPolicyReplacementRequired {
		t.Fatalf("unsupportedForceNew final policy = %q, want %q", unresolvedForceNew.Merge.FinalPolicy, mutabilityOverlayPolicyReplacementRequired)
	}
	if !containsDiagnostic(unresolvedForceNew.Provenance.Notes, `did not resolve to exactly one canonical join key`) {
		t.Fatalf("unsupportedForceNew notes = %v, want unresolved-join detail", unresolvedForceNew.Provenance.Notes)
	}
}
