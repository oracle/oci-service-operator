/*
  Copyright (c) 2021, Oracle and/or its affiliates. All rights reserved.
  Licensed under the Universal Permissive License v 1.0 as shown at http://oss.oracle.com/licenses/upl.
*/

// Code generated by generator. DO NOT EDIT.

package workrequest

import (
	"context"

	generativeaisdk "github.com/oracle/oci-go-sdk/v65/generativeai"
	generativeaiv1beta1 "github.com/oracle/oci-service-operator/api/generativeai/v1beta1"
	generatedruntime "github.com/oracle/oci-service-operator/pkg/servicemanager/generatedruntime"
)

type runtimeOperationHooks[Req any, Resp any] struct {
	Fields []generatedruntime.RequestField
	Call   func(context.Context, Req) (Resp, error)
}

type WorkRequestRuntimeHooks struct {
	Semantics       *generatedruntime.Semantics
	BuildCreateBody func(context.Context, *generativeaiv1beta1.WorkRequest, string) (any, error)
	BuildUpdateBody func(context.Context, *generativeaiv1beta1.WorkRequest, string, any) (any, bool, error)
	// Identity owns bounded pre-create guard and identity reuse hooks.
	Identity            generatedruntime.IdentityHooks[*generativeaiv1beta1.WorkRequest]
	Read                generatedruntime.ReadHooks
	TrackedRecreate     generatedruntime.TrackedRecreateHooks[*generativeaiv1beta1.WorkRequest]
	StatusHooks         generatedruntime.StatusHooks[*generativeaiv1beta1.WorkRequest]
	ParityHooks         generatedruntime.ParityHooks[*generativeaiv1beta1.WorkRequest]
	Async               generatedruntime.AsyncHooks[*generativeaiv1beta1.WorkRequest]
	DeleteHooks         generatedruntime.DeleteHooks[*generativeaiv1beta1.WorkRequest]
	Get                 runtimeOperationHooks[generativeaisdk.GetWorkRequestRequest, generativeaisdk.GetWorkRequestResponse]
	List                runtimeOperationHooks[generativeaisdk.ListWorkRequestsRequest, generativeaisdk.ListWorkRequestsResponse]
	WrapGeneratedClient []func(WorkRequestServiceClient) WorkRequestServiceClient
}

type WorkRequestRuntimeHooksMutator func(*WorkRequestServiceManager, *WorkRequestRuntimeHooks)

var workrequestRuntimeHooksMutators []WorkRequestRuntimeHooksMutator

func registerWorkRequestRuntimeHooksMutator(mutator WorkRequestRuntimeHooksMutator) {
	if mutator == nil {
		return
	}
	workrequestRuntimeHooksMutators = append(workrequestRuntimeHooksMutators, mutator)
}
func newWorkRequestDefaultRuntimeHooks(sdkClient generativeaisdk.GenerativeAiClient) WorkRequestRuntimeHooks {
	return WorkRequestRuntimeHooks{
		Identity:        generatedruntime.IdentityHooks[*generativeaiv1beta1.WorkRequest]{},
		Read:            generatedruntime.ReadHooks{},
		TrackedRecreate: generatedruntime.TrackedRecreateHooks[*generativeaiv1beta1.WorkRequest]{},
		StatusHooks:     generatedruntime.StatusHooks[*generativeaiv1beta1.WorkRequest]{},
		ParityHooks:     generatedruntime.ParityHooks[*generativeaiv1beta1.WorkRequest]{},
		Async:           generatedruntime.AsyncHooks[*generativeaiv1beta1.WorkRequest]{},
		DeleteHooks:     generatedruntime.DeleteHooks[*generativeaiv1beta1.WorkRequest]{},
		Get: runtimeOperationHooks[generativeaisdk.GetWorkRequestRequest, generativeaisdk.GetWorkRequestResponse]{
			Fields: []generatedruntime.RequestField{{FieldName: "WorkRequestId", RequestName: "workRequestId", Contribution: "path", PreferResourceID: true}},
			Call: func(ctx context.Context, request generativeaisdk.GetWorkRequestRequest) (generativeaisdk.GetWorkRequestResponse, error) {
				return sdkClient.GetWorkRequest(ctx, request)
			},
		},
		List: runtimeOperationHooks[generativeaisdk.ListWorkRequestsRequest, generativeaisdk.ListWorkRequestsResponse]{
			Fields: []generatedruntime.RequestField{{FieldName: "CompartmentId", RequestName: "compartmentId", Contribution: "query", PreferResourceID: false}, {FieldName: "WorkRequestId", RequestName: "workRequestId", Contribution: "query", PreferResourceID: false}, {FieldName: "Status", RequestName: "status", Contribution: "query", PreferResourceID: false}, {FieldName: "ResourceId", RequestName: "resourceId", Contribution: "query", PreferResourceID: false}, {FieldName: "Page", RequestName: "page", Contribution: "query", PreferResourceID: false}, {FieldName: "Limit", RequestName: "limit", Contribution: "query", PreferResourceID: false}, {FieldName: "SortOrder", RequestName: "sortOrder", Contribution: "query", PreferResourceID: false}, {FieldName: "SortBy", RequestName: "sortBy", Contribution: "query", PreferResourceID: false}},
			Call: func(ctx context.Context, request generativeaisdk.ListWorkRequestsRequest) (generativeaisdk.ListWorkRequestsResponse, error) {
				return sdkClient.ListWorkRequests(ctx, request)
			},
		},
		WrapGeneratedClient: []func(WorkRequestServiceClient) WorkRequestServiceClient{},
	}
}

func newWorkRequestRuntimeHooks(manager *WorkRequestServiceManager, sdkClient generativeaisdk.GenerativeAiClient) WorkRequestRuntimeHooks {
	hooks := newWorkRequestDefaultRuntimeHooks(sdkClient)
	for _, mutator := range workrequestRuntimeHooksMutators {
		mutator(manager, &hooks)
	}
	return hooks
}

func buildWorkRequestGeneratedRuntimeConfig(
	manager *WorkRequestServiceManager,
	hooks WorkRequestRuntimeHooks,
) generatedruntime.Config[*generativeaiv1beta1.WorkRequest] {
	return generatedruntime.Config[*generativeaiv1beta1.WorkRequest]{
		Kind:            "WorkRequest",
		SDKName:         "WorkRequest",
		Log:             manager.Log,
		Semantics:       hooks.Semantics,
		Identity:        hooks.Identity,
		Read:            hooks.Read,
		TrackedRecreate: hooks.TrackedRecreate,
		StatusHooks:     hooks.StatusHooks,
		ParityHooks:     hooks.ParityHooks,
		Async:           hooks.Async,
		DeleteHooks:     hooks.DeleteHooks,
		BuildCreateBody: hooks.BuildCreateBody,
		BuildUpdateBody: hooks.BuildUpdateBody,
		Get: &generatedruntime.Operation{
			NewRequest: func() any { return &generativeaisdk.GetWorkRequestRequest{} },
			Fields:     append([]generatedruntime.RequestField(nil), hooks.Get.Fields...),
			Call: func(ctx context.Context, request any) (any, error) {
				return hooks.Get.Call(ctx, *request.(*generativeaisdk.GetWorkRequestRequest))
			},
		},
		List: &generatedruntime.Operation{
			NewRequest: func() any { return &generativeaisdk.ListWorkRequestsRequest{} },
			Fields:     append([]generatedruntime.RequestField(nil), hooks.List.Fields...),
			Call: func(ctx context.Context, request any) (any, error) {
				return hooks.List.Call(ctx, *request.(*generativeaisdk.ListWorkRequestsRequest))
			},
		},
	}
}

func wrapWorkRequestGeneratedClient(
	hooks WorkRequestRuntimeHooks,
	delegate WorkRequestServiceClient,
) WorkRequestServiceClient {
	wrapped := delegate
	for _, wrap := range hooks.WrapGeneratedClient {
		if wrap == nil {
			continue
		}
		wrapped = wrap(wrapped)
	}
	return wrapped
}
