/*
  Copyright (c) 2021, Oracle and/or its affiliates. All rights reserved.
  Licensed under the Universal Permissive License v 1.0 as shown at http://oss.oracle.com/licenses/upl.
*/

// Code generated by generator. DO NOT EDIT.

package certificate

import (
	"context"
	"fmt"

	waassdk "github.com/oracle/oci-go-sdk/v65/waas"
	waasv1beta1 "github.com/oracle/oci-service-operator/api/waas/v1beta1"
	"github.com/oracle/oci-service-operator/pkg/servicemanager"
	generatedruntime "github.com/oracle/oci-service-operator/pkg/servicemanager/generatedruntime"
	ctrl "sigs.k8s.io/controller-runtime"
)

// CertificateServiceClient is the handwritten extension seam for Certificate runtime behavior.
// Add a manual file in this package that registers runtime hook mutators or wires a custom client through
// (*CertificateServiceManager).WithClient.
type CertificateServiceClient interface {
	CreateOrUpdate(context.Context, *waasv1beta1.Certificate, ctrl.Request) (servicemanager.OSOKResponse, error)
	Delete(context.Context, *waasv1beta1.Certificate) (bool, error)
}

type defaultCertificateServiceClient struct {
	generatedruntime.ServiceClient[*waasv1beta1.Certificate]
}

var _ CertificateServiceClient = defaultCertificateServiceClient{}

var newCertificateServiceClient = func(manager *CertificateServiceManager) CertificateServiceClient {
	sdkClient, err := waassdk.NewWaasClientWithConfigurationProvider(manager.Provider)
	hooks := newCertificateRuntimeHooks(manager, sdkClient)
	config := buildCertificateGeneratedRuntimeConfig(manager, hooks)
	if err != nil {
		config.InitError = fmt.Errorf("initialize Certificate OCI client: %w", err)
	}
	delegate := defaultCertificateServiceClient{
		ServiceClient: generatedruntime.NewServiceClient[*waasv1beta1.Certificate](config),
	}
	return wrapCertificateGeneratedClient(hooks, delegate)
}
