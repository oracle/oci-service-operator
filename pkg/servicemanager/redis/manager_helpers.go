...
const redisRequeueDuration = 30 * time.Second
...
func resolveClusterID(statusID, specID ociv1beta1.OCID) (ociv1beta1.OCID, error) {
	...
	return "", fmt.Errorf("redis cluster ocid is empty")
}
...
func reconcileLifecycleStatus(status *ociv1beta1.OSOKStatus, cluster *redis.RedisCluster,
	...
	case redis.RedisClusterLifecycleStateActive:
		...
		*status = util.UpdateOSOKStatusCondition(*status, ociv1beta1.Active, v1.ConditionTrue, "",
			fmt.Sprintf("RedisCluster %s is %s", safeString(cluster.DisplayName), cluster.LifecycleState), log)
		return servicemanager.OSOKResponse{IsSuccessful: true}
	case redis.RedisClusterLifecycleStateCreating,
		redis.RedisClusterLifecycleStateUpdating:
