...
type runtimeCheckRunInputs struct {
	...
	manualServiceManagerRoots   []string
}
...
func loadRuntimeCheckRunInputs(ctx context.Context, opts options) (runtimeCheckRunInputs, error) {
	...
	return runtimeCheckRunInputs{
		...
		manualServiceManagerRoots:   manualServiceManagerRoots(services),
	}, nil
}
...
func generateRuntimeCheckSnapshot(ctx context.Context, inputs runtimeCheckRunInputs, opts options, snapshotRoot string) error {
	...
	if err := preserveManualServiceManagerRoots(inputs.repoRoot, snapshotRoot, inputs.manualServiceManagerRoots); err != nil {
		return fmt.Errorf("preserve manual service-manager roots in runtime snapshot: %w", err)
	}
	return nil
}
...
func manualServiceManagerRoots(services []generator.ServiceConfig) []string {
	seen := make(map[string]struct{})
	for _, service := range services {
...
func preserveManualServiceManagerRoots(repoRoot, snapshotRoot string, roots []string) error {
	for _, root := range roots {
		root = strings.TrimSpace(root)
		if root == "" {
			continue
		}
		sourcePath := filepath.Join(repoRoot, "pkg", "servicemanager", root)
		destPath := filepath.Join(snapshotRoot, "pkg", "servicemanager", root)
		if _, err := os.Lstat(destPath); err == nil {
			continue
		} else if !os.IsNotExist(err) {
			return err
		}
		if err := symlinkIfPresent(sourcePath, destPath); err != nil {
			return err
		}
	}
	return nil
