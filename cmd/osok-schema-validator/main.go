...
func main() {
	...
	flag.StringVar(&opts.ConfigPath, "config", opts.ConfigPath, "Optional generator config path used to scope validation to selected services/kinds")
	flag.StringVar(&opts.Service, "service", opts.Service, "Optional service filter (for example: core, identity, psql)")
	flag.BoolVar(&opts.All, "all", opts.All, "Scope to the enabled default-active services from the generator config")
