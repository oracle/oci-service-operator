...
func init() {
	...
	utilruntime.Must(computeinstanceagentv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "computeinstanceagent",
		LeaderElectionID:   "40558063.oci.computeinstanceagent",
		SkipFIPS:           true,
	}, managerservices.ForGroup("computeinstanceagent")); err != nil {
		os.Exit(1)
	}
}
