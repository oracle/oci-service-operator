...
func init() {
	...
	utilruntime.Must(functionsv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "functions",
		LeaderElectionID:   "40558063.oci.functions",
		SkipFIPS:           true,
	}, managerservices.ForGroup("functions")); err != nil {
		os.Exit(1)
	}
}
