...
func init() {
	...
	utilruntime.Must(filestoragev1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "filestorage",
		LeaderElectionID:   "40558063.oci.filestorage",
		SkipFIPS:           true,
	}, managerservices.ForGroup("filestorage")); err != nil {
		os.Exit(1)
	}
}
