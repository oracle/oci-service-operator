...
func init() {
	...
	utilruntime.Must(dataflowv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "dataflow",
		LeaderElectionID:   "40558063.oci.dataflow",
		SkipFIPS:           true,
	}, managerservices.ForGroup("dataflow")); err != nil {
		os.Exit(1)
	}
}
