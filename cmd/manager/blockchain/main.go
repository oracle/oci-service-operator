...
func init() {
	...
	utilruntime.Must(blockchainv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "blockchain",
		LeaderElectionID:   "40558063.oci.blockchain",
		SkipFIPS:           true,
	}, managerservices.ForGroup("blockchain")); err != nil {
		os.Exit(1)
	}
}
