...
func init() {
	...
	utilruntime.Must(stackmonitoringv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "stackmonitoring",
		LeaderElectionID:   "40558063.oci.stackmonitoring",
		SkipFIPS:           true,
	}, managerservices.ForGroup("stackmonitoring")); err != nil {
		os.Exit(1)
	}
}
