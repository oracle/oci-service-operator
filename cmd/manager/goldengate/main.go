...
func init() {
	...
	utilruntime.Must(goldengatev1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "goldengate",
		LeaderElectionID:   "40558063.oci.goldengate",
		SkipFIPS:           true,
	}, managerservices.ForGroup("goldengate")); err != nil {
		os.Exit(1)
	}
}
