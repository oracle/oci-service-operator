...
func init() {
	...
	utilruntime.Must(managementagentv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "managementagent",
		LeaderElectionID:   "40558063.oci.managementagent",
		SkipFIPS:           true,
	}, managerservices.ForGroup("managementagent")); err != nil {
		os.Exit(1)
	}
}
