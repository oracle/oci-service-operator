...
func init() {
	...
	utilruntime.Must(databasemigrationv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "databasemigration",
		LeaderElectionID:   "40558063.oci.databasemigration",
		SkipFIPS:           true,
	}, managerservices.ForGroup("databasemigration")); err != nil {
		os.Exit(1)
	}
}
