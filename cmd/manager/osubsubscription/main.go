...
func init() {
	...
	utilruntime.Must(osubsubscriptionv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "osubsubscription",
		LeaderElectionID:   "40558063.oci.osubsubscription",
		SkipFIPS:           true,
	}, managerservices.ForGroup("osubsubscription")); err != nil {
		os.Exit(1)
	}
}
