...
func init() {
	...
	utilruntime.Must(vnmonitoringv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "vnmonitoring",
		LeaderElectionID:   "40558063.oci.vnmonitoring",
		SkipFIPS:           true,
	}, managerservices.ForGroup("vnmonitoring")); err != nil {
		os.Exit(1)
	}
}
