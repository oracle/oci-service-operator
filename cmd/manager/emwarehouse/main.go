...
func init() {
	...
	utilruntime.Must(emwarehousev1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "emwarehouse",
		LeaderElectionID:   "40558063.oci.emwarehouse",
		SkipFIPS:           true,
	}, managerservices.ForGroup("emwarehouse")); err != nil {
		os.Exit(1)
	}
}
