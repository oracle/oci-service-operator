...
func init() {
	...
	utilruntime.Must(managedkafkav1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "managedkafka",
		LeaderElectionID:   "40558063.oci.managedkafka",
		SkipFIPS:           true,
	}, managerservices.ForGroup("managedkafka")); err != nil {
		os.Exit(1)
	}
}
