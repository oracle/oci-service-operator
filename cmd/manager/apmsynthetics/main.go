...
func init() {
	...
	utilruntime.Must(apmsyntheticsv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "apmsynthetics",
		LeaderElectionID:   "40558063.oci.apmsynthetics",
		SkipFIPS:           true,
	}, managerservices.ForGroup("apmsynthetics")); err != nil {
		os.Exit(1)
	}
}
