...
func init() {
	...
	utilruntime.Must(cloudmigrationsv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "cloudmigrations",
		LeaderElectionID:   "40558063.oci.cloudmigrations",
		SkipFIPS:           true,
	}, managerservices.ForGroup("cloudmigrations")); err != nil {
		os.Exit(1)
	}
}
