...
func init() {
	...
	utilruntime.Must(dashboardservicev1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "dashboardservice",
		LeaderElectionID:   "40558063.oci.dashboardservice",
		SkipFIPS:           true,
	}, managerservices.ForGroup("dashboardservice")); err != nil {
		os.Exit(1)
	}
}
