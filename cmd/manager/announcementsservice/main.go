...
func init() {
	...
	utilruntime.Must(announcementsservicev1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "announcementsservice",
		LeaderElectionID:   "40558063.oci.announcementsservice",
		SkipFIPS:           true,
	}, managerservices.ForGroup("announcementsservice")); err != nil {
		os.Exit(1)
	}
}
