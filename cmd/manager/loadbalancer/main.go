...
func init() {
	...
	utilruntime.Must(loadbalancerv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "loadbalancer",
		LeaderElectionID:   "40558063.oci.loadbalancer",
		SkipFIPS:           true,
	}, managerservices.ForGroup("loadbalancer")); err != nil {
		os.Exit(1)
	}
}
