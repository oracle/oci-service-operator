...
func init() {
	...
	utilruntime.Must(marketplacepublisherv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "marketplacepublisher",
		LeaderElectionID:   "40558063.oci.marketplacepublisher",
		SkipFIPS:           true,
	}, managerservices.ForGroup("marketplacepublisher")); err != nil {
		os.Exit(1)
	}
}
