...
func init() {
	...
	utilruntime.Must(onesubscriptionv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "onesubscription",
		LeaderElectionID:   "40558063.oci.onesubscription",
		SkipFIPS:           true,
	}, managerservices.ForGroup("onesubscription")); err != nil {
		os.Exit(1)
	}
}
