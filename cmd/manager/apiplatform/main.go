...
func init() {
	...
	utilruntime.Must(apiplatformv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "apiplatform",
		LeaderElectionID:   "40558063.oci.apiplatform",
		SkipFIPS:           true,
	}, managerservices.ForGroup("apiplatform")); err != nil {
		os.Exit(1)
	}
}
