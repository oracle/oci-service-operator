...
func init() {
	...
	utilruntime.Must(accessgovernancecpv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "accessgovernancecp",
		LeaderElectionID:   "40558063.oci.accessgovernancecp",
		SkipFIPS:           true,
	}, managerservices.ForGroup("accessgovernancecp")); err != nil {
		os.Exit(1)
	}
}
