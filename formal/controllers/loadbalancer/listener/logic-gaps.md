...
- Create, update, and delete track the Load Balancing work request before performing a `GetLoadBalancer`-backed read. Because the child has no independent lifecycle field, the package-local projection marks a listener found on its ACTIVE parent as `ACTIVE` so reconciliation settles deterministically without accepting stale post-write state.
