...
- Update is field-aware and repo-authored. The runtime reads the live Queue by tracked OCID, rejects create-only drift for `compartmentId` and `retentionInSeconds`, allows mutable `UpdateQueueDetails` fields including `capabilities`, forwards repo-authored capability payloads when the enabled capability type set changes or the controller-owned consumer-group capability-detail snapshot drifts, and preserves the explicit empty-string custom-encryption-key clear path.
- Status projection is part of the checked-in contract. The runtime projects live OCI Queue fields and the observable enabled capability types into the published `QueueStatus` surface, rehydrates the remembered consumer-group capability payload into `status.capabilities[*].jsonData` after successful ACTIVE reconciles, uses `status.async.current` as the canonical in-flight operation tracker, and keeps `createWorkRequestId`, `updateWorkRequestId`, and `deleteWorkRequestId` stable as compatibility mirrors across requeues.
- Queue capability parity is controller-owned for the non-observable consumer-group add-on fields. OCI `GetQueue` still does not echo the consumer-group settings used when a capability is enabled, so steady-state drift detection compares the live enabled capability type set plus the controller-owned last-applied consumer-group payload snapshot instead of looping forever on provider-silent subfields.
