...
- Queue capability parity is intentionally type-based after observe. OCI `GetQueue` does not echo the consumer-group add-on settings used when a capability is enabled, so steady-state drift detection compares the enabled capability type set instead of looping forever on request-only subfields that the service does not publish back.
