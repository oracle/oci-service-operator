...
SCHEMA_VALIDATOR_CONFIG ?= $(EFFECTIVE_GENERATOR_CONFIG)
...
SCHEMA_VALIDATOR_EFFECTIVE_SELECTION = $(strip $(if $(SCHEMA_VALIDATOR_SELECTION),$(SCHEMA_VALIDATOR_SELECTION),$(if $(strip $(SCHEMA_VALIDATOR_UPGRADE_FROM)$(SCHEMA_VALIDATOR_UPGRADE_TO)),supported,full)))
SCHEMA_VALIDATOR_SELECTION_ARG = $(if $(filter supported,$(SCHEMA_VALIDATOR_EFFECTIVE_SELECTION)),--config $(SCHEMA_VALIDATOR_CONFIG) $(if $(strip $(SCHEMA_VALIDATOR_SERVICE)),,--all),)
SCHEMA_VALIDATOR_TMPDIR ?= $(patsubst %/,%,$(or $(TMPDIR),/tmp))
SCHEMA_VALIDATOR_EFFECTIVE_FORMAT = $(strip $(if $(SCHEMA_VALIDATOR_FORMAT),$(SCHEMA_VALIDATOR_FORMAT),$(if $(strip $(SCHEMA_VALIDATOR_UPGRADE_FROM)$(SCHEMA_VALIDATOR_UPGRADE_TO)),markdown,json)))
SCHEMA_VALIDATOR_EFFECTIVE_REPORT = $(strip $(if $(SCHEMA_VALIDATOR_REPORT),$(SCHEMA_VALIDATOR_REPORT),$(if $(strip $(SCHEMA_VALIDATOR_UPGRADE_FROM)$(SCHEMA_VALIDATOR_UPGRADE_TO)),$(SCHEMA_VALIDATOR_TMPDIR)/validator-upgrade-report.md,validator-report.json)))
SCHEMA_VALIDATOR_UPGRADE_CONFIG ?= $(EFFECTIVE_GENERATOR_CONFIG)
SCHEMA_VALIDATOR_UPGRADE_PROVIDER_PATH ?= .
SCHEMA_VALIDATOR_UPGRADE_FROM ?=
SCHEMA_VALIDATOR_UPGRADE_TO ?=
SCHEMA_VALIDATOR_UPGRADE_FORMAT ?= markdown
SCHEMA_VALIDATOR_UPGRADE_REPORT ?= $(SCHEMA_VALIDATOR_TMPDIR)/validator-upgrade-report.md
SCHEMA_VALIDATOR_UPGRADE_SERVICE ?=
SCHEMA_VALIDATOR_UPGRADE_ARG = $(if $(strip $(SCHEMA_VALIDATOR_UPGRADE_FROM)$(SCHEMA_VALIDATOR_UPGRADE_TO)),--upgrade-from $(SCHEMA_VALIDATOR_UPGRADE_FROM) --upgrade-to $(SCHEMA_VALIDATOR_UPGRADE_TO),)
FORMAL_ROOT ?= formal
...
schema-validator: ## Run OSOK schema validator (coverage by default, SDK upgrade diff when SCHEMA_VALIDATOR_UPGRADE_FROM/TO are set).
	@case "$(SCHEMA_VALIDATOR_EFFECTIVE_SELECTION)" in full|supported) ;; *) echo "Set SCHEMA_VALIDATOR_SELECTION=full|supported" && exit 1 ;; esac
	@if [ -n "$(strip $(SCHEMA_VALIDATOR_UPGRADE_FROM)$(SCHEMA_VALIDATOR_UPGRADE_TO))" ]; then \
		test -n "$(SCHEMA_VALIDATOR_UPGRADE_FROM)" || (echo "Set SCHEMA_VALIDATOR_UPGRADE_FROM=v65.x.y" && exit 1); \
		test -n "$(SCHEMA_VALIDATOR_UPGRADE_TO)" || (echo "Set SCHEMA_VALIDATOR_UPGRADE_TO=v65.x.y" && exit 1); \
	fi
	@report="$(SCHEMA_VALIDATOR_EFFECTIVE_REPORT)"; \
	report_dir="$$(dirname "$$report")"; \
	tmp_report="$$report.tmp.$$PPID"; \
	mkdir -p "$$report_dir"; \
	rm -f "$$tmp_report"; \
	if go run ./cmd/osok-schema-validator --provider-path $(SCHEMA_VALIDATOR_PROVIDER_PATH) $(SCHEMA_VALIDATOR_SELECTION_ARG) $(SCHEMA_VALIDATOR_SERVICE_ARG) $(SCHEMA_VALIDATOR_UPGRADE_ARG) --format $(SCHEMA_VALIDATOR_EFFECTIVE_FORMAT) > "$$tmp_report"; then \
		mv "$$tmp_report" "$$report"; \
		echo "Wrote schema validator report to $$report"; \
	else \
		rm -f "$$tmp_report"; \
		exit 1; \
	fi

schema-validator-upgrade: ## Backward-compatible alias for a supported-surface SDK upgrade diff.
	@$(MAKE) schema-validator \
		SCHEMA_VALIDATOR_PROVIDER_PATH="$(SCHEMA_VALIDATOR_UPGRADE_PROVIDER_PATH)" \
		SCHEMA_VALIDATOR_CONFIG="$(SCHEMA_VALIDATOR_UPGRADE_CONFIG)" \
		SCHEMA_VALIDATOR_SELECTION=supported \
		SCHEMA_VALIDATOR_SERVICE="$(SCHEMA_VALIDATOR_UPGRADE_SERVICE)" \
		SCHEMA_VALIDATOR_FORMAT="$(SCHEMA_VALIDATOR_UPGRADE_FORMAT)" \
		SCHEMA_VALIDATOR_REPORT="$(SCHEMA_VALIDATOR_UPGRADE_REPORT)" \
		SCHEMA_VALIDATOR_UPGRADE_FROM="$(SCHEMA_VALIDATOR_UPGRADE_FROM)" \
		SCHEMA_VALIDATOR_UPGRADE_TO="$(SCHEMA_VALIDATOR_UPGRADE_TO)"
