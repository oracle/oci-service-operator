/*
  Copyright (c) 2021, Oracle and/or its affiliates. All rights reserved.
  Licensed under the Universal Permissive License v 1.0 as shown at http://oss.oracle.com/licenses/upl.
*/

// Code generated by generator. DO NOT EDIT.

package v1beta1

import (
	shared "github.com/oracle/oci-service-operator/pkg/shared"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

// AttributeTagSpec defines the desired state of AttributeTag.
type AttributeTagSpec struct {
	// The name of the tag in the case of a free form tag.
	// When linking to a glossary term, this field is not specified.
	// +kubebuilder:validation:Optional
	Name string `json:"name,omitempty"`
	// Unique key of the related term or null in the case of a free form tag.
	// +kubebuilder:validation:Optional
	TermKey string `json:"termKey,omitempty"`
	// The OCID of the catalog containing the tagged attribute.
	// +kubebuilder:validation:Required
	CatalogId string `json:"catalogId"`
	// The key of the data asset containing the tagged attribute.
	// +kubebuilder:validation:Required
	DataAssetKey string `json:"dataAssetKey"`
	// The key of the entity containing the tagged attribute.
	// +kubebuilder:validation:Required
	EntityKey string `json:"entityKey"`
	// The key of the attribute receiving the tag.
	// +kubebuilder:validation:Required
	AttributeKey string `json:"attributeKey"`
}

// AttributeTagStatus defines the observed state of AttributeTag.
type AttributeTagStatus struct {
	OsokStatus shared.OSOKStatus `json:"status"`
	// Unique tag key that is immutable.
	Key string `json:"key,omitempty"`
	// Name of the tag which matches the term name.
	Name string `json:"name,omitempty"`
	// Unique key of the related term.
	TermKey string `json:"termKey,omitempty"`
	// Path of the related term.
	TermPath string `json:"termPath,omitempty"`
	// Description of the related term.
	TermDescription string `json:"termDescription,omitempty"`
	// The current state of the tag.
	LifecycleState string `json:"lifecycleState,omitempty"`
	// The date and time the tag was created, in the format defined by RFC3339 (https://tools.ietf.org/html/rfc3339).
	// Example: `2019-03-25T21:10:29.600Z`
	TimeCreated string `json:"timeCreated,omitempty"`
	// OCID of the user who created the tag.
	CreatedById string `json:"createdById,omitempty"`
	// URI to the tag instance in the API.
	Uri string `json:"uri,omitempty"`
	// The unique key of the parent attribute.
	AttributeKey string `json:"attributeKey,omitempty"`
	// Unique id of the parent glossary of the term.
	GlossaryKey string `json:"glossaryKey,omitempty"`
}

// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:printcolumn:name="Name",type="string",JSONPath=".spec.name",priority=1
// +kubebuilder:printcolumn:name="Status",type="string",JSONPath=".status.status.conditions[-1].type",description="status of the AttributeTag",priority=0
// +kubebuilder:printcolumn:name="Ocid",type="string",JSONPath=".status.status.ocid",description="Ocid of the AttributeTag",priority=1
// +kubebuilder:printcolumn:name="Age",type="date",JSONPath=".metadata.creationTimestamp",priority=0
// AttributeTag is the Schema for the attributetags API.
type AttributeTag struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   AttributeTagSpec   `json:"spec,omitempty"`
	Status AttributeTagStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true
// AttributeTagList contains a list of AttributeTag.
type AttributeTagList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []AttributeTag `json:"items"`
}

func init() {
	SchemeBuilder.Register(&AttributeTag{}, &AttributeTagList{})
}
